...
def get_command_line_args():
    ...
    parser.add_argument('--year', type=int, required=True, help='ICML year to search.')
    parser.add_argument('--month', type=int, help='ICML month. (Optinal)')
    ...
    if args.year > 2020:
        ...
        raise ValueError("Year < 2013 not supported.")
    year = args.year
...
def main():
    ...
    print("Loading ICML {} results".format(year))
    
    try:
        driver.get('http://proceedings.mlr.press/{}'.format(pmlr))
    ...
    # Save results
    data_ranked.to_csv(os.path.join(csvpath, 'ICML{}'.format(year)+'.csv'), encoding='utf-8') # Change the path
